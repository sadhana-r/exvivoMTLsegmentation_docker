...
from nnunet.network_architecture.neural_network_SOR import SegmentationNetwork #Change back to _SOR
...
class SuccessiveOverRelaxation(nn.Module):
    ...
    def forward(self, image):
        ...
        #output_file = '/data/sadhanar/Example_Initialization_' + str(r) + '.nii.gz'
        #save_img = init[0,:].squeeze().float()
        ...
        #output_file = '/data/sadhanar/Example_Black.nii.gz'
        #save_img = black[0,:].squeeze().float()
        #nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)

        #output_file = '/data/sadhanar/Example_Red.nii.gz'
        #save_img = red[0,:].squeeze().float()
        #nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)
        ...
        #Set WM/CSF to zero - Don't do
        #init = init*mask_gm

        #laplace_onehot = convert_laplacian_toseg(init)
        laplace_onehot = convert_laplacian_toseg_finer(init)
        laplace_onehot = laplace_onehot.cuda()
...
class Generic_UNet_SOR(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool,num_conv_per_stage=2,
                 ...
                 seg_output_use_bias=False, sor_start_epoch = 10, sor_source_label = None, sor_sink_label = None, sor_num_iterations = 60, compute_sor = False, num_laplace_thresholds = 11):
        """
        ...
        """
        super(Generic_UNet_SOR, self).__init__()
        ...
        self.num_thresholds = num_laplace_thresholds

    def update_epoch(self, epoch):
        self.current_epoch = epoch
        if self.current_epoch > self.sor_start_epoch:
            self.compute_sor = True


    def update_SORiterations(self,num_iterations):
        self.sor_module = SuccessiveOverRelaxation(source = self.source_label,
                sink = self.sink_label, max_iterations = num_iterations)
    ...
    def forward(self, x):
        ...
        output_file = '/data/sadhanar/Example_MRPatch_' + str(r) + '.nii.gz'
        save_img = x[0,:].squeeze().float()
        nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)

        output_file = '/data/sadhanar/Example_GTseg_' + str(r) + '.nii.gz'
        save_img = t[1][0,:].squeeze().float()
        nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)

        output_file = '/data/sadhanar/Example_GTLaplace_' + str(r) + '.nii.gz'
        save_img = t[0][0,:].squeeze().float()
        nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)
        """

        #Just adding this for prediction of network on test scans (MTLsegmentaion - May 2022). Need to modify code properly. 
        x = x[:,0,:]
        x = x[:,None,:]
        ...
        if self._deep_supervision and self.do_ds and not self.compute_sor:
            ...
            #Soft argmax - to scale values to probabilities (0/1)
            if torch.isnan(softmax_helper(2*seg_output_clone)).any():
                print("softmax output contains nan:", torch.isnan(softmax_helper(2*seg_output_clone)).any())
            
            seg_output_softmax = softmax_helper(seg_output_clone*2)

            #output_file = '/data/sadhanar/Example_ProbMap_' + str(r) + '.nii.gz'
            #save_img = seg_output_softmax[0,:].squeeze().float()
            #nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)

            laplace_field, laplace_onehot  = self.sor_module(seg_output_softmax)

            #output_file = '/data/sadhanar/Example_LaplaceField_' + str(r) + '.nii.gz'
            #save_img = laplace_field[0,:].squeeze().float()
            #nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)

            #output_file = '/data/sadhanar/Example_LaplaceOneHot_' + str(r) + '.nii.gz'
            #save_img = laplace_onehot[0,:].squeeze().float()
            #nib.save(nib.Nifti1Image(save_img.detach().cpu().numpy(), np.eye(4)),output_file)

            self.num_thresholds = laplace_onehot.shape[1]
