...
class DownsampleSegForDSTransform2(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        data_dict[self.output_key] = downsample_seg_for_ds_transform2_SR(data_dict[self.input_key], self.ds_scales,
...
def downsample_seg_for_ds_transform2_SR(seg, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25)), order=0, axes=None):
    ...
    laplacian_target = seg[:,1:]
    output.append(laplacian_target) # Laplacian map is the first channel. It is also the first channel of the output

    #Actual segmentation is the second channel onwards
    seg = seg[:,0,:]
